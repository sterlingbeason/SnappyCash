...
const ignoreNodeList = ['SCRIPT', 'COMMENT', 'STYLE', 'DIV']; // ignore these textNodes Parents
...
function quickFilterNode(node) {
    try {
        let returnValue =   typeof node.data === 'undefined' || // assumption textNode should have 'data' property
                            ...
                            node.parentNode.hasOwnProperty('contentEditable'); // elements that act as inputs (likely would be a div already filterd in ignore list)
        return returnValue;
    } catch (error) {
